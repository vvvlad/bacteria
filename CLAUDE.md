...
```
TIFF stack ──► io.load_stack() ──► (T, Y, X) ndarray
    │
    ▼
...
`matching.py` handles a separate cross-modality path: aligning phase-contrast cell masks with fluorescence nucleus masks via overlap then nearest-centroid fallback.
...
- **Frame trimming**: `TRIM_FRAMES` parameter in the notebook slices off initial frames before detection. `FRAME_OFFSET` converts analysis frame indices back to raw frame numbers.
...
- `TRIM_FRAMES`, `MERGE_MAX_DISTANCE`, `MERGE_MAX_GAP` control frame trimming and track merging.
